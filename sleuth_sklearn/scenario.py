...
class Scenario:
    ...
    def __str__(self):
        ...
        vars_list = [f"{key}={str(value)}" for key, value in vars(self).items()]
        vars_str = "\n".join(vars_list)
        return f"{name}\n{vars_str}"
    ...
    def init_coeffs(self, config, field):
        paramlist = ["DIFFUSION", "SPREAD", "BREED", "SLOPE", "ROAD"]
        ...
        for param in paramlist:
            ...
            #     continue
            if field in isfloat:
                coeffs[param.lower()] = config[param].getfloat(field, fallback=0.0)
            else:
                coeffs[param.lower()] = config[param].getint(field, fallback=0)
        return coeffs
